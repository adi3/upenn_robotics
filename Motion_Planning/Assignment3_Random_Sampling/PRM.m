...
for i = 2:nsamples
    ...
    [k_dist, k_indices] = mink(distances, k);
    for idx = 1:size(k_indices)
        if LocalPlanner(x, samples(:,k_indices(idx)))
            nedges = nedges + 1;
            edges(nedges, 1) = i;
            edges(nedges, 2) = k_indices(idx);
            edge_lengths(nedges) = k_dist(idx);
        end
    end
